...
export class BranchMasterComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  createForm() {
    this.form = this.fb.group({
      ...
      short_name: [''],
      smtp_host: ['', [Validators.pattern(/^[a-zA-Z0-9.-]+$/)]],
      smtp_username: [''],
      smtp_password:['', [Validators.required, Validators.minLength(6)]],
      smtp_email: ['', [Validators.email]],
      smtp_port: ['', [ Validators.pattern('^[0-9]{2,5}$')]],
      smtp_ssl: [true],
      wp_token:   ['', [Validators.pattern(/^[0-9]+$/),   Validators.minLength(10)]],  
      sms_username: [''],
      sms_password: [''],
      sms_sender:['', [ Validators.maxLength(6)]],
      footer: [''],
      id: [0],
    })
  }
